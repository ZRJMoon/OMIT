...
def get_args():
    ...
    parser.add_argument("--droot", type=str, default='', help="root to dataset")
...
class Trainer:
    def __init__(self, args, config, device):
        ...
        # dataloaders
        self.train_loader = get_fourier_train_dataloader(args=self.args, config=self.config, alpha=self.config['alpha'])
    ...
    def compute_rl_matrix(self, features):
        features = F.normalize(features, dim=-1)
        rela = torch.matmul(features, features.transpose(0,1))
        return rela
    ...
    def _do_epoch(self):
        ...
        for it, (batch, label, domain) in enumerate(self.train_loader):
            ...
            features, features_dist = self.encoder(batch)
            scores, scores_dist = self.classifier(features, features_dist)
            rl_ori = self.compute_rl_matrix(features_dist[:batch.shape[0]//2])
            rl_aug = self.compute_rl_matrix(features_dist[batch.shape[0]//2:])
            with torch.no_grad():
                features_teacher, features_dist_teacher = self.encoder_teacher(batch)
                scores_teacher, scores_dist_teacher = self.classifier_teacher(features_teacher, features_dist_teacher)
                scores_teacher = (scores_teacher + scores_dist_teacher) / 2

            assert batch.size(0) % 2 == 0
            split_idx = int(batch.size(0) / 2)
            scores_ori, scores_aug = torch.split(scores, split_idx)
            scores_dist_ori, scores_dist_aug = torch.split(scores_dist, split_idx)
            ...
            p_rl_ori, p_rl_aug = F.softmax(rl_ori/ self.config["T_rl"], dim=1), F.softmax(rl_aug/ self.config["T_rl"], dim=1)
            loss_rl_ori = F.kl_div(p_rl_aug.log(), p_rl_ori.detach(), reduction='batchmean')
            loss_rl_aug = F.kl_div(p_rl_ori.log(), p_rl_aug.detach(), reduction='batchmean') 
            ...
            p_dist_ori, p_dist_aug = F.softmax(scores_dist_ori / self.config["T_kl"], dim=1), F.softmax(scores_dist_aug / self.config["T_kl"], dim=1)
            p_ori_tea, p_aug_tea = F.softmax(scores_ori_tea / self.config["T_kl"], dim=1), F.softmax(scores_aug_tea / self.config["T_kl"], dim=1)

            # use KLD for consistency loss
            loss_ori_tea = F.kl_div(p_dist_aug.log(), p_ori_tea, reduction='batchmean')
            loss_aug_tea = F.kl_div(p_dist_ori.log(), p_aug_tea, reduction='batchmean')

            # get consistency weight
            const_weight_kl = get_current_consistency_weight(epoch=self.current_epoch,
                                                          weight=self.config["lam_const_kl"],
                                                          rampup_length=self.config["warmup_epoch"],
                                                          rampup_type=self.config["warmup_type"])
            
            const_weight_rl = get_current_consistency_weight(epoch=self.current_epoch,
                                                          weight=self.config["lam_const_rl"],
                                                          rampup_length=self.config["warmup_epoch"],
                                                          rampup_type=self.config["warmup_type"])

            # calculate total loss
            total_loss = 0.5 * loss_cls + 0.5 * loss_aug + \
                         const_weight_kl * loss_ori_tea + const_weight_kl * loss_aug_tea + \
                         const_weight_rl * loss_rl_ori + const_weight_rl * loss_rl_aug

            loss_dict["ori_tea"] = loss_ori_tea.item()
            loss_dict["aug_tea"] = loss_aug_tea.item()
            loss_dict["ori_rl"] = loss_rl_ori.item()
            loss_dict["aug_rl"] = loss_rl_aug.item()
