...
def get_dataset(data_root, path, train=False, image_size=224, crop=False, jitter=0, config=None):
    names, labels = dataset_info(data_root, path)
    if config:
...
def get_fourier_dataset(data_root, path, image_size=224, crop=False, jitter=0, from_domain='all', alpha=1.0, config=None):
    assert isinstance(path, list)
    names = []
    labels = []
    for p in path:
        name, label = dataset_info(data_root, p)
        names.append(name)
        labels.append(label)
