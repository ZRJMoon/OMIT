...
def get_fourier_train_dataloader(
    ...
    for dname in source_list:
        ...
        path = os.path.join(datalists_folder, '%s_train.txt' % dname)
        paths.append(path)
    dataset = get_fourier_dataset(data_root=data_root, path=paths,
                                  image_size=image_size,
                                  crop=crop,
                                  jitter=jitter,
...
def get_val_dataloader(source_list=None, batch_size=64, image_size=224, args=None, config=None):
    ...
    for dname in source_list:
        ...
        val_dataset = get_dataset(data_root=data_root, path=path, train=False, image_size=image_size, config=data_config)
...
def get_test_loader(target=None, batch_size=64, image_size=224, args=None, config=None):
    ...
    test_dataset = get_dataset(data_root=data_root, path=path, train=False, image_size=image_size, config=data_config)
