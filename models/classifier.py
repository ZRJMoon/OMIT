...
class Classifier(nn.Module):
    def __init__(self, in_dim, num_classes, bias=False, scale=1.0, learn_scale=False, cls_type="linear"):
        ...
        if self.classifier_type == "linear":
            ...
            self.layers_dist = nn.Linear(in_dim, num_classes)
    ...
    def forward(self, features, features_dist=None):
        scores = self.layers(features)
        if self.training:
            scores_dist = self.layers_dist(features_dist)
            return scores, scores_dist
        return scores
